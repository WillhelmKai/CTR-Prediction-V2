...
#second GRU for img
img_second_GRU_input = tf.reshape(tf.multiply(img_attention_output, img_GRU_outputs), [-1,500,1]) #[deepth, 1100]
...
NN_input = tf.concat([tf.transpose(img_final_state_second),tf.transpose(final_state_second),ph_candidate_categories,ph_candidate_brand,ph_candidate_price], 1)
NN_input = norm(NN_input, 5865)
W_fc_1 = weight_variable([5865, 200])
